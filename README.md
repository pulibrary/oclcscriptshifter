...
The "Exclude subfields" section allows you to specify that certain subfields should always be excluded from conversion (because they likely to not contain text in the language being converted.)  Subfields can be indicated with the three-digit tag number followed by the subfield code, with 'x' as the wildcord.  So, in the example above, subfield 'e' of any 1xx or 7xx field will be excluded, as well as 856u.  Subfields with numeric codes (0, 2, 6, etc.) are always excluded from conversion.
